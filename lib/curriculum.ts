import { BAC_TRACKS, type Chapter, type TrackProgram, type TrackId as BacTrackId } from './bac-curriculum'
import { ALL_QUESTIONS, type Question as BacQuestion } from './bac-questions'
...
export type TrackId = 'code' | 'python' | BacTrackId
...
export type Track = {
  ...
  color: string
  lessons: Lesson[]
  /** Marks a bonus track (Code/Python) that is not part of the official BAC programme. */
  bonus?: boolean
}
...
/** Map BAC chapter IDs (or demo chapter IDs) to widget specs. */
function widgetForChapter(track: BacTrackId, chapterId: string): WidgetSpec | undefined {
  // Physics
  if (chapterId === 'ph-projectile') return { type: 'projectile' }
  if (chapterId === 'ph-pendule') return { type: 'pendule' }
  if (chapterId === 'ph-orbites') return { type: 'orbites' }
  // Maths
  if (chapterId === 'ma-grapheur') return { type: 'grapheur' }
  if (chapterId === 'ma-cercle') return { type: 'cercle-trigo' }
  if (chapterId === 'ma-tangente') return { type: 'tangente' }
  // NSI / Python
  if (track === 'nsi') return { type: 'python', task: 'Écris une fonction Python et exécute-la pour vérifier le comportement.', starter: '# Ton code ici\\n', expected: '', hint: 'Utilise Python directement dans le bac à sable.' }
  if (chapterId === 'ma-python') return { type: 'python', task: 'Implémente un calcul scientifique en Python (suites, Monte-Carlo, dérivée numérique).', starter: '# Écris ta simulation\\n', expected: '', hint: 'Pense à utiliser range, sum et les bibliothèques standards.' }
  return undefined
}

/* ============================================================
   Bonus tracks (kept from legacy curriculum)
   ============================================================ */
  ...
  lessons: [
    {
      ...
      widget: { type: 'python', task: "Fais afficher exactement « Bonjour le BAC » par le programme.", starter: '# Écris ton premier instruction ci-dessous\nprint("Bonjour")\n', expected: 'Bonjour le BAC', hint: 'Remplace le texte entre guillemets par : Bonjour le BAC' },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: "Crée a = 7 et b = 3, puis fais afficher leur somme et leur produit sur une même ligne : « 10 21 ».", starter: 'a = 7\nb = 3\n# affiche la somme puis le produit séparés par un espace\n', expected: '10 21', hint: 'print(a + b, a * b) affiche les deux résultats séparés par un espace.' },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: "Avec note = 14, écris une condition qui affiche « Admis » si la note est supérieure ou égale à 10, sinon « Ajourné ». Ici le programme doit afficher « Admis ».", starter: 'note = 14\n# écris la condition ci-dessous\n', expected: 'Admis', hint: "if note >= 10:\n    print(\"Admis\")\nelse:\n    print(\"Ajourné\")" },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: "Fais afficher les nombres de 1 à 5, un par ligne, avec une boucle for.", starter: '# utilise range et une boucle for\n', expected: '1\n2\n3\n4\n5', hint: 'for i in range(1, 6):\n    print(i)' },
      quiz: [
...
const PYTHON_TRACK: Track = {
  id: 'python',
  title: 'Bonus · Python : niveau suivant',
  ...
  lessons: [
    {
      ...
      widget: { type: 'python', task: "Écris une fonction aire(l, L) qui renvoie l'aire d'un rectangle, puis affiche aire(5, 4). Résultat attendu : « 20 ».", starter: '# définis aire puis affiche aire(5, 4)\n', expected: '20', hint: 'def aire(l, L):\n    return l * L\n\nprint(aire(5, 4))' },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: "Avec notes = [12, 15, 9], affiche la somme puis le maximum séparés par un espace (« 36 15 »).", starter: 'notes = [12, 15, 9]\n', expected: '36 15', hint: 'print(sum(notes), max(notes))' },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: 'Avec mot = "BAC", affiche le mot en minuscules puis sa longueur séparés par un espace (« bac 3 »).', starter: 'mot = "BAC"\n', expected: 'bac 3', hint: 'print(mot.lower(), len(mot))' },
      quiz: [
    ...
    },
    {
      ...
      widget: { type: 'python', task: "Calcule la moyenne de notes = [12, 15, 9, 18] et affiche-la arrondie à 2 décimales. Attendu : « 13.5 ».", starter: 'notes = [12, 15, 9, 18]\n# calcule et affiche la moyenne\n', expected: '13.5', hint: 'm = sum(notes) / len(notes)\nprint(round(m, 2))' },
      quiz: [
...
/* ============================================================
   Physics + maths lessons with sim widgets (legacy, kept)
   ============================================================ */

const PHYSICS_LEGACY: Lesson[] = [
  {
    id: 'ph-projectile',
...
/* ============================================================
   Build official BAC tracks (each chapter → Lesson)
   ============================================================ */

/** Quiz for a chapter: 3-5 questions pulled from bac-questions.ts. If fewer, synthesize. */
function quizForChapter(trackId: BacTrackId, chapter: Chapter): QuizQuestion[] {
  const matching: BacQuestion[] = ALL_QUESTIONS.filter((q) => q.trackId === trackId && q.chapterId === chapter.id)
  if (matching.length >= 3) {
    // Pick a representative spread: 4 questions across difficulties if possible.
    const sorted = [...matching].sort((a, b) => a.difficulty - b.difficulty)
    const picks: BacQuestion[] = []
    const step = sorted.length / Math.min(4, sorted.length)
    for (let i = 0; i < Math.min(4, sorted.length); i++) picks.push(sorted[Math.floor(i * step)])
    return picks.map((q) => ({ q: q.q, options: q.options, answer: q.answer }))
  }
  // Synthesize a tiny quiz from keyConcepts so every chapter has SOMETHING to validate.
  const concepts = chapter.keyConcepts.slice(0, 3)
  if (concepts.length === 0) {
    return [{ q: `As-tu compris l'essentiel de « ${chapter.title} » ?`, options: ['Oui, complètement', 'En grande partie', 'Pas encore'], answer: 0 }]
  }
  return concepts.map((c) => ({
    q: `Coche l'affirmation exacte concernant : « ${chapter.title} ».`,
    options: [c, `${chapter.title} ne figure pas au programme officiel.`, `Ce chapitre ne contient aucune notion clé.`],
    answer: 0,
  }))
}

function lessonFromChapter(trackId: BacTrackId, chapter: Chapter, index: number): Lesson {
  const widget = widgetForChapter(trackId, chapter.id)
  return {
    id: chapter.id,
    title: chapter.title,
    minutes: chapter.estimatedMinutes,
    intro: [
      ...chapter.objectives.map((o) => `→ ${o}`),
      ...chapter.keyConcepts.map((k) => `• ${k}`),
    ],
    keyPoints: chapter.keyConcepts,
    widget,
    quiz: quizForChapter(trackId, chapter),
    bacTrack: trackId,
  }
}

function bacLessons(trackId: BacTrackId): Lesson[] {
  const track = BAC_TRACKS.find((t) => t.id === trackId)
  if (!track) return []
  return track.chapters.map((c, i) => lessonFromChapter(trackId, c, i))
}

function bacTrack(trackId: BacTrackId): Track {
  const t = BAC_TRACKS.find((x) => x.id === trackId)!
  return {
    id: trackId,
    title: t.title,
    tagline: t.tagline,
    color: t.color,
    lessons: bacLessons(trackId),
  }
}

/* ============================================================
   Special merge: Physique gets its legacy sim lessons prepended
   before the BAC chapters so users see the sims first.
   ============================================================ */

const PHYSIQUE_TRACK: Track = {
  id: 'physique',
  title: BAC_TRACKS.find((t) => t.id === 'physique')!.title,
  tagline: BAC_TRACKS.find((t) => t.id === 'physique')!.tagline,
  color: BAC_TRACKS.find((t) => t.id === 'physique')!.color,
  lessons: [...PHYSICS_LEGACY, ...bacLessons('physique')],
}

const MATHS_TRACK: Track = {
  id: 'maths',
  title: BAC_TRACKS.find((t) => t.id === 'maths')!.title,
  tagline: BAC_TRACKS.find((t) => t.id === 'maths')!.tagline,
  color: BAC_TRACKS.find((t) => t.id === 'maths')!.color,
  lessons: [...MATHS_LEGACY, ...bacLessons('maths')],
}

/* ============================================================
   Final TRACKS list (8 official + 2 bonus)
   ============================================================ */
...
export const BADGES: Badge[] = [
  ...
  { id: 'svt', label: 'Naturaliste', description: 'Finir toute la piste SVT' },
  { id: 'philo', label: 'Philosophe', description: 'Finir toute la piste Philosophie' },
  { id: 'francais', label: 'Plume française', description: 'Finir toute la piste Français' },
  { id: 'histoire', label: 'Mémoire vive', description: 'Finir toute la piste Histoire-Géo' },
  { id: 'ses', label: 'Économiste en herbe', description: 'Finir toute la piste SES' },
  { id: 'nsi', label: 'Numérique & sciences', description: 'Finir toute la piste NSI' },
  { id: 'marathon', label: 'Marathon', description: 'Terminer 10 leçons' },
...
export function earnedBadges(completions: { lessonId: string; score: number }[]): Set<string> {
  ...
  if (ids.size >= 10) out.add('marathon')
  ...
  if (doneAll('svt')) out.add('svt')
  if (doneAll('philo')) out.add('philo')
  if (doneAll('francais')) out.add('francais')
  if (doneAll('histoire')) out.add('histoire')
  if (doneAll('ses')) out.add('ses')
  if (doneAll('nsi')) out.add('nsi')
